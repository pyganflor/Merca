...
<body class="hold-transition skin-black layout-top-nav">
    <div class="wrapper">
        ...
        @include('layouts.adminlte_cliente.menu_superior')
        ...
        <footer class="main-footer">
            ...
            <strong>Copyright &copy; 2018-{{ date('Y') }} Benchflow S.A.</strong> All rights
            reserved.
        </footer>
    ...
    <script src="{{ url('adminlte/bower_components/jquery-ui/jquery-ui-touch.min.js') }}"></script>
    ...
    <script>
        ...
        function modal_quest(id_modal, mensaje, title, draggable, closable, size, accion) {
            ...
            BootstrapDialog.show({
                ...
                onshown: function(modal) {
                    ...
                    $('#btn_continue_' + id_modal).attr('data-dismiss', 'modal');
                },
                callback: function() {
                    arreglo_modals_form = [];
                },
        ...
        function buscar_notificaciones(tipo, push = true) {
            ...
            $.post('notificaciones/buscar_notificaciones', datos, function(retorno) {
                ...
                if (retorno.cant_news > 0) {
                    retorno.cant_news == 1 ? texto = 'Usted tiene 1 notificación' : texto = 'Usted tiene ' + retorno
                        .cant_news + ' notificaciones';
                    ...
                    if (push == true) {
                        for (i = 0; i < retorno.array.length; i++) {
                            notificar(retorno.array[i]['titulo'], true, function() {
        ...
        function delete_user_notification(id) {
            ...
            $('#btn_notificaciones').LoadingOverlay('show');
            $.post('{{ url('notificaciones/delete_user_notification') }}', datos, function(retorno) {

            }, 'json').fail(function(retorno) {
                console.log(retorno);
                alerta_errores(retorno.responseText);
            }).always(function() {
                $('#btn_notificaciones').LoadingOverlay('hide');
            })
        }
        ...
        function add_documento(entidad, codigo, accion) {
            ...
            get_jquery('{{ url('documento/add_documento') }}', datos, function(retorno) {
                modal_form('modal_add_documento', retorno,
                    '<i class="fa fa-fw fa-object-group"></i> Añadir información personalizada', true, false,
                    '{{ isPC() ? '50%' : '' }}',
                    function() {
        ...
        function delete_documento(id, codigo, entidad) {
            modal_quest('modal_quest_del_documento',
                '<div class="alert alert-info text-center">¿Está seguro de eliminar esta información?</div>',
        ...
        function select_planta_global(p, input_variedad, elemento_load, li_adicional = '', f) {
            if (p != '') {
                datos = {
                    planta: p
                };
                get_jquery('{{ url('select_planta_global') }}', datos, function(retorno) {
                    $('#' + input_variedad).html(li_adicional);
                    $('#' + input_variedad).append(retorno);
                }, elemento_load);
        ...
        function select_plantaByFinca(finca, p, input_variedad, elemento_load, li_adicional = '', f) {
            if (p != '' && finca != '') {
                datos = {
                    planta: p,
                    finca: finca,
                };
                get_jquery('{{ url('select_plantaByFinca') }}', datos, function(retorno) {
                    $('#' + input_variedad).html(li_adicional);
                    $('#' + input_variedad).append(retorno);
                }, elemento_load);
        ...
        function ver_rendimiento_cosecha(id_cosecha) {
            if (id_cosecha != '') {
                ...
                get_jquery('{{ url('recepcion/ver_rendimiento') }}', datos, function(retorno) {
                    modal_view('modal_view_ver_rendimiento', retorno,
                        '<i class="fa fa-fw fa-balance-scale"></i> Rendimiento', true, false,
                        '{{ isPC() ? '75%' : '' }}');
                });
            }
        }
        ...
        function cargar_fincas_propias() {
            datos = {};
            get_jquery('{{ url('cargar_fincas_propias') }}', datos, function(retorno) {
                $('#ul_navbar_superior').append(retorno);
            }, 'ul_navbar_superior');
        }
        ...
        function cargar_utiles() {
            get_jquery('{{ url('cargar_utiles') }}', {}, function(retorno) {
                modal_view('modal-view_cargar_utiles', retorno,
                    '<i class="fa fa-fw fa-code"></i> Reportes de desarrollo', true, false,
                    '{{ isPC() ? '60%' : '' }}');
            })
        }
        ...
        function mini_alerta(icon, texto, timer = '') {
            ...
            MiniAlert.fire({
                icon: icon,
                html: '<h5 class="text-center">' + texto + '</h5>'
            });
        }

        function select_submenu_crm(value) {
            if (value == 'dashboard_inicial') {
                $.LoadingOverlay('show');
                location.href = '{{ url('dashboard') }}' + '?f=' + $('#fincas_propias').val();
            } else {
                cargar_url(value);
            }
        }
        ...
        $(window).resize(function() {
            if ($(document).width() >= 1024 && $('#count_accesos_directos').val() <=
                2) { // mostrar accesos directos completos
