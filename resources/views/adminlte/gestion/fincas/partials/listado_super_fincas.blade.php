<table class="table-bordered table-striped" style="width: 100%; border: 1px solid #9d9d9d; border-radius: 18px 18px 0 0"
    ...
    <tbody>
        @foreach ($listado as $item)
            <tr id="tr_super_finca_{{ $item->id_super_finca }}">
                <td class="text-center" style="border-color: #9d9d9d">
                    <input type="text" id="nombre_sf_{{ $item->id_super_finca }}" value="{{ $item->nombre }}"
                        style="width: 100%; padding-left: 5px">
                </td>
                <td class="text-center" style="border-color: #9d9d9d">
                    <div class="btn-group">
                        <button type="button" class="btn btn-yura_primary btn-xs"
                            onclick="update_super_finca('{{ $item->id_super_finca }}')">
...
<script>
    function store_super_finca() {
        ...
        post_jquery_m('{{ url('fincas/store_super_finca') }}', datos, function() {
            listar_fincas();
            listar_super_fincas();
        });
    }
    ...
    function update_super_finca(sf) {
        ...
        post_jquery_m('{{ url('fincas/update_super_finca') }}', datos, function() {
            listar_fincas();
        }, 'tr_super_finca_' + sf);
    }
</script>
