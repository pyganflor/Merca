<script>
    ...
    function buscar_listado() {
        ...
        $.get('{{ url('usuarios/buscar') }}', datos, function(retorno) {
            ...
            $('#table_content_usuarios_filter>label>input').addClass('input-yura_default');
        }).always(function() {
            $.LoadingOverlay('hide');
        });
    }
    ...
    function eliminar_usuario(id, estado) {
        mensaje = {
            title: estado == 'A' ? '<i class="fa fa-fw fa-trash"></i> Desactivar usuario' :
                '<i class="fa fa-fw fa-unlock"></i> Activar usuario',
            mensaje: estado == 'A' ?
                '<div class="alert alert-danger text-center"><i class="fa fa-fw fa-exclamation-triangle"></i> ¿Está seguro de desactivar este usuario?</div>' :
        ...
        modal_quest('modal_delete_usuario', mensaje['mensaje'], mensaje['title'], true, false,
            '{{ isPC() ? '25%' : '' }}',
            function() {
                datos = {
                    _token: '{{ csrf_token() }}',
