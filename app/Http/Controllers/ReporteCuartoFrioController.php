...
class ReporteCuartoFrioController extends Controller
{
            ...
            foreach ($variedades as $v) {
                ...
                foreach ($longitudes as $l) {
                    ...
                    if ($request->tallos_x_ramo != '')
                        $cantidad = $cantidad->where('i.tallos_x_ramo', $request->tallos_x_ramo);
                    if ($finca_destino == 'F')
    ...
    public function get_importar_file_bajas(Request $request)
    {
        ...
        foreach ($sheet as $pos => $row) {
            if ($pos > 1) {
                $row_A = espacios($row['A']);
                $nombre_variedad = mb_strtoupper(espacios(explode(' (', $row_A)[0]));
                $longitud = mb_strtolower(espacios(explode(' (', $row_A)[1]));
