...
class UsuarioController extends Controller
{
    ...
    public function config_user_finca(Request $request)
    {
        $empresas = ConfiguracionEmpresa::All()
            ->where('proveedor', 0)
            ->sortByDesc('estado');
        $usuario = getUsuario($request->user);
        return view('adminlte.gestion.usuarios.forms.config_user_finca', [
            'usuario' => $usuario,
    ...
    public function store_finca_user(Request $request)
    {
        $usuario = getUsuario($request->user);
        $anteriores = $usuario->empresas;
        foreach ($anteriores as $i)
            $i->delete();
        foreach ($request->data as $d) {
            $user_finca = new UsuarioFinca();
            $user_finca->id_usuario = $request->user;
            $user_finca->id_empresa = $d;
            $user_finca->fecha_registro = date('Y-m-d H:i:s');
            if ($user_finca->save()) {
                $user_finca = UsuarioFinca::All()->last();
                bitacora('usuario_finca', $user_finca->id_usuario_finca, 'I', 'Insercion de un usuario_finca');
            } else {
                return [
                    'success' => false,
