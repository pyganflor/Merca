...
{
    return [
        ['inicio' => '00:00', 'fin' => '01:00'], ['inicio' => '01:00', 'fin' => '02:00'], ['inicio' => '02:00', 'fin' => '03:00'],
...
function getUltimoDiaMes($anno, $mes)
{
    if (((fmod($anno, 4) == 0) and (fmod($anno, 100) != 0)) or (fmod($anno, 400) == 0)) {
        $dias_febrero = 29;
    } else {
        $dias_febrero = 28;
    }
...
function getInicioFinLastSemana()
{
    $last_lunes = date('Y-m-d', strtotime('last Monday', time()));
    $inicio = opDiasFecha('-', 7, $last_lunes);
    $fin = opDiasFecha('-', 1, $last_lunes);
    return [
        'inicio' => $inicio,
        'fin' => $fin,
    ];
...
function getGrupoMenusOfUser($usuario)
{
    $grupos = DB::table('grupo_menu as g')
        ...
        ->select('g.id_grupo_menu', 'g.nombre')->distinct()
...
function getSubmenusOfUser($usuario)
{
    $submenus = DB::table('submenu as s')
        ->join('rol_submenu as rs', 's.id_submenu', '=', 'rs.id_submenu')
        ->join('menu as m', 'm.id_menu', '=', 's.id_menu')
        ->join('grupo_menu as g', 'g.id_grupo_menu', '=', 'm.id_grupo_menu')
        ->select('s.nombre', 's.url', 's.tipo')->distinct()
        ->where('rs.id_rol', '=', getUsuario(Session::get('id_usuario'))->id_rol)
        ->where('rs.estado', '=', 'A')
        ->where('g.estado', '=', 'A')
        ->where('m.estado', '=', 'A')
...
{
    return Variedad::where('estado', '=', 1)->get();
}
...
{
    return Planta::All()->where('estado', 1)->where('tipo', $tipo)->sortBy('nombre');
}
...
function getSemanasByCodigos($desde, $hasta)
{
    $r = DB::select('select distinct codigo, fecha_inicial, fecha_final
        from semana
        where codigo >= ' . $desde .
        ' and codigo <= ' . $hasta .
        ' order by codigo');
...
function getSemanasByCodigosVariedad($desde, $hasta, $variedad)
{
    $r = DB::select('select distinct codigo, fecha_inicial, fecha_final
        from semana
        where codigo >= ' . $desde .
        ' and codigo <= ' . $hasta .
        ' and id_variedad = ' . $variedad .
        ' order by codigo');
    return $r;
}
...
function getSemanaByDate($fecha)
{
    ...
    $r = DB::select("SELECT * FROM semana WHERE semana_guia = 1 AND fecha_inicial <= '" . $fecha . "' AND fecha_final >= '" . $fecha . "' LIMIT 1");
    $r = count($r) > 0 ? $r[0] : '';
    $r = $r != '' ? Semana::find($r->id_semana) : '';
...
{
    exec(
        "java -Dfile.encoding=UTF-8 -jar " . env('PATH_JAR_FIRMADOR') . " "
...
function enviarComprobante($comprobante_xml, $clave_acceso, $carpeta)
{
    ...
    exec(
        'java -Dfile.encoding=UTF-8 -jar ' . env('PATH_JAR_ENVIADOR') . ' '
            . env('PATH_XML_FIRMADOS') . $carpeta . " "
            . $comprobante_xml . " "
...
function getCantidadRamosPedidosForCB(
    $fecha,
    $variedad,
    $clasificacion_ramo, /*$envoltura, */
    $presentacion,
    $tallos_x_ramos,
    $longitud_ramo,
    $unidad_medida
...
function getDisponibleInventarioFrio(
    $variedad,
    $clasificacion_ramo, /*$envoltura, */
    $presentacion,
    $tallos_x_ramos,
...
function getAreaCiclosByRango($semana_ini, $semana_fin, $variedad, $id_empresa = 'T')
{
    $variedades = [];
    ...
    if (count($semanas) > 0) {
        ...
        if ($id_empresa != 'T')
            $query_variedades = $query_variedades->where('id_empresa', $id_empresa);

        $query_variedades = $query_variedades->get();
        ...
        foreach ($query_variedades as $var) {
            if ($var->id_variedad == $variedad || $variedad == 'T') {
                ...
                $query = $query->orderBy('fecha_inicio')
                    ->get();
                ...
                array_push($variedades, [
                    'variedad' => getVariedad($var->id_variedad),
                    'ciclos' => $ciclos
                ]);
            }
        }
...
function getCiclosCerradosByRango($semana_ini, $semana_fin, $variedad, $by_semana = true, $finca = '')
{
    $finca = $finca != '' ? $finca : getFincaActiva();
    if ($by_semana) {
...
function getTallosCosechadosByModRangoVar($mod, $desde, $hasta, $var)
{
...
function getTallosCosechadosByFechaVarModFinca($fecha, $variedad, $mod, $finca)
{
    $r = DB::table('desglose_recepcion as dr')
...
{
    $r = DB::select("SELECT * FROM semana WHERE semana_guia = 1 AND codigo = '" . $codigo . "' LIMIT 1");
    $r = count($r) > 0 ? $r[0] : '';
    $r = Semana::find($r->id_semana);
    return $r;
}

function getObjSemanaVariedad($codigo, $variedad)
{
    $r = DB::select("SELECT * FROM semana WHERE codigo = '" . $codigo . "' AND id_variedad = " . $variedad . " LIMIT 1");
    $r = count($r) > 0 ? $r[0] : '';
    $r = Semana::find($r->id_semana);
    return $r;
}
...
function getUsuarioFincaByFincas($fincas, $f)
{
    foreach ($fincas as $finc)
        if ($finc->id_empresa == $f)
            return $finc;
    return '';
}

function getFincasPropias()
{
    return getUsuario(Session::get('id_usuario'))->empresas;
}

function getAllFincas()
{
    return ConfiguracionEmpresa::All();
}
...
function getCamaById($id)
{
    return Cama::find($id);
}

function calcularCamas($m2)
{
    return round($m2 / 48, 2);
}

function getFincaActiva()
{
    return getUsuario(Session::get('id_usuario'))->finca_activa;
}
...
function mensaje_desarrollo()
{
    return '<div class="alert alert-info text-center text-black"><h2><i class="fa fa-fw fa-gears"></i> Estamos trabajando en esta pantalla en estos momentos</h2></div>';
}

function getPlantasPropag($finca)
{
    $plantas_ptas_madres = DB::table('ciclo_cama as cc')
    ...
    $ids_plantas_pm = [];
    foreach ($plantas_ptas_madres as $item)
        array_push($ids_plantas_pm, $item->id_planta);
    $plantas_enrz = DB::table('enraizamiento_semanal as e')
        ->join('variedad as v', 'v.id_variedad', '=', 'e.id_variedad')
        ->join('planta as p', 'p.id_planta', '=', 'v.id_planta')
        ->select('v.id_planta', 'p.nombre')->distinct()
        ->where('e.semana_fin', '>=', getSemanaByDate(hoy())->codigo)
        ->where('e.id_empresa', $finca)
        ->where('v.estado', 1)
        ->where('p.estado', 1)
        ->whereNotIn('v.id_planta', $ids_plantas_pm)
        ->orderBy('p.nombre')->get();
    foreach ($plantas_enrz as $item)
        array_push($ids_plantas_pm, $item->id_planta);
    $plantas = DB::table('planta')
        ->select('id_planta', 'nombre')->distinct()
        ->whereIn('id_planta', $ids_plantas_pm)
        ->orderBy('nombre')->get();
    return $plantas;
}

function getVariedadesPropag($finca)
{
    $vars_ptas_madres = DB::table('ciclo_cama as cc')
        ->join('variedad as v', 'v.id_variedad', '=', 'cc.id_variedad')
        ->join('planta as p', 'p.id_planta', '=', 'v.id_planta')
        ->select('cc.id_variedad')->distinct()
        ->where('cc.activo', 1)
        ->where('cc.id_empresa', $finca)
        ->where('v.estado', 1)
        ->where('p.estado', 1)
    ...
    $ids_vars_pm = [];
    foreach ($vars_ptas_madres as $item)
        array_push($ids_vars_pm, $item->id_variedad);
    $vars_enrz = DB::table('enraizamiento_semanal as e')
        ->join('variedad as v', 'v.id_variedad', '=', 'e.id_variedad')
        ->join('planta as p', 'p.id_planta', '=', 'v.id_planta')
        ->select('e.id_variedad')->distinct()
        ->where('e.semana_fin', '>=', getSemanaByDate(hoy())->codigo)
        ->where('e.id_empresa', $finca)
        ->where('v.estado', 1)
        ->where('p.estado', 1)
        ->whereNotIn('v.id_planta', $ids_vars_pm)
        ->orderBy('v.nombre')->get();
    foreach ($vars_enrz as $item)
        array_push($ids_vars_pm, $item->id_variedad);
    $listado = DB::table('variedad as v')
        ->join('planta as p', 'p.id_planta', '=', 'v.id_planta')
        ->select('v.id_variedad as id_variedad', 'p.nombre as planta', 'v.nombre as variedad')->distinct()
        ->whereIn('v.id_variedad', $ids_vars_pm)
        ->orderBy('v.nombre')->get();
    return [
        'variedades' => $listado,
        'ids' => $ids_vars_pm,
...
function getColumnasExcel()
{
    ...
    foreach ($default as $x)
        foreach ($default as $y)
            $columnas[] = $x . $y;

    return $columnas;
}

function setBgToCeldaExcel($sheet, $celda, $color)
{
    $sheet->getStyle($celda)->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID)->getStartColor()->setRGB($color);
}

function setColorTextToCeldaExcel($sheet, $celda, $color)
{
    $sheet->getStyle($celda)->getFont()->getColor()->setRGB($color);
}
...
function setBorderToCeldaExcel($sheet, $celda)
{
...
function getQueueForProyNoPerenne($variedad)
{
    $finca = getFincaActiva();
    $query = DB::table('jobs')
        ->select('queue')->distinct()
        ->where('queue', 'like', 'proy_no_perennes%')
        ->where('payload', 'like', '%jobProyNoPerennes%variedad%";i:' . $variedad . '%empresa%";i:' . $finca . '%')
        ->orderBy('fecha_registro')
        ->get();
    if (count($query) > 0)
        return $query[0]->queue;

    $menor = [
        'cant' => '',
        'queue' => 'proy_no_perennes',
    ];
    for ($i = 1; $i <= 10; $i++) {
        $j = Job::where('queue', 'proy_no_perennes_' . $i)
            ->get();
        $count = count($j);
        if ($count == 0) {
            return 'proy_no_perennes_' . $i;
        } else {
            if ($menor['cant'] == '' || $menor['cant'] > $count) {
                $menor['cant'] = $count;
                $menor['queue'] = 'proy_no_perennes_' . $i;
            }
        }
    }
    return $menor['queue'];
...
function getQueueForProyPerenne($variedad)
{
    $finca = getFincaActiva();
    $query = DB::table('jobs')
        ->select('queue')->distinct()
        ->where('queue', 'like', 'proy_perennes%')
        ->where('payload', 'like', '%jobActualizarSemProyPerenne%variedad%";i:' . $variedad . '%finca%";i:' . $finca . '%')
        ->orderBy('fecha_registro')
        ->get();
    if (count($query) > 0)
        return $query[0]->queue;
    ...
    for ($i = 1; $i <= 10; $i++) {
        $j = Job::where('queue', 'proy_perennes_' . $i)
            ->get();
        $count = count($j);
        if ($count == 0) {
            return 'proy_perennes_' . $i;
        } else {
            if ($menor['cant'] == '' || $menor['cant'] > $count) {
                $menor['cant'] = $count;
                $menor['queue'] = 'proy_perennes_' . $i;
            }
        }
    }
    return $menor['queue'];
...
function getPromMonitoreoCalibreByFecha($fecha)
{
    ...
    $calibre = DB::table('monitoreo_calibre as m')
        ...
        ->where('m.fecha', $fecha)
        ->get()[0]->calibre;
    return porcentaje(100 - $desecho, $calibre, 2);
}

function getAplicacionCampoById($id)
{
    return AplicacionCampo::find($id);
}

function getDelta10Dias()
{
